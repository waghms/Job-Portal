...
<head>
    ...
    <title>Job Portal Users List</title>
        ...
        <div class="main-panel">
            ...
            <div class="container">
                <div class="page-inner">
                    ...
                    <form method="GET" action="{{ route('admin.profiles.index') }}" class="mb-4">
						<div class="row">
							<!-- Search input -->
							<div class="col-12 col-md-8">
								<input type="text" name="search" class="form-control" placeholder="Search Profiles..." value="{{ $search }}">
							</div>
							<!-- Search button -->
                            ...
                            <tbody>
                                @if(isset($profiles) && $profiles->count() > 0)
                                    @foreach($profiles as $index => $profile)
                                        <tr>
                                            <td>{{ ($profiles->currentPage() - 1) * $profiles->perPage() + $index + 1 }}</td>
                                            <td>{{ ($profile->firstname ?? 'N/A') . ' ' . ($profile->lastname ?? '') }}</td>
                                            <td>{{ $profile->dob ?? 'N/A' }}</td>
                                            <td>{{ $profile->email ?? 'N/A' }}</td>
                                            <td>{{ $profile->gender ?? 'N/A' }}</td>
                                            <td>{{ $profile->address ?? 'N/A' }}</td>
                                            <td>{{ $profile->contact_no ?? 'N/A' }}</td>
                                        </tr>
                                    @endforeach
                                @else
                                    <tr>
