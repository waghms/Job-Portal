...
<head>
	...
	<title>Job Portal Applications list</title>
			...
			<div class="container">
                <div class="page-inner">
                    <div class="page-header">
                        <h3 class="fw-bold mb-3">All Applications</h3>
								...
								<form method="GET" action="{{ route('admin.applications.index') }}" class="mb-4">
									<div class="row">
										...
										<div class="col-12 col-md-8">
											<input type="text" name="search" class="form-control" placeholder="Search Applications..." value="{{ $search }}">
										</div>
										<!-- Search button -->
										...
										<tbody>
											@if(isset($applications) && $applications->count() > 0)
												@foreach($applications as $index => $application)
													<tr>
														<td>{{ ($applications->currentPage() - 1) * $applications->perPage() + $index + 1 }}</td> <!-- Adjusted SR -->
														<td>{{ ($application->firstname ?? 'N/A') . ' ' . ($application->lastname ?? '') }}</td>
														<td>{{ $application->jobroll ?? 'N/A' }}</td>
														<td>{{ \Carbon\Carbon::parse($application->created_at)->format('M d, Y, h:i A') }}</td>
														<td>{{ $application->marks ?? 'N/A' }}</td>
														<td>
															<span class="badge 
														...
														<td>
															<form action="{{ route('admin.updateApplicationStatus', $application->id) }}" method="POST">
																@csrf
																<div class="d-flex align-items-center">
																	<select name="status" class="form-control me-2" id="test" style="padding: 0.4rem;">
